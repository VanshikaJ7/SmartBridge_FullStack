...
const Stories=()=>{
    ...
    const fetchStories=async()=>{
        try{
            const response=await axios.get("http://localhost:6001/api/story/fetchAllStories");
...
const handleOpenStory=async(story)=>{
    setStory(story);
    await socket.emit('story-played',{storyId:story._id,userId:localStorage.getItem('uerId')})
...
return(
    ...
    <div className="storiesBody" style={isStoryPlaying?{display:'none'}:{}}>
        <div className="stories">
            ...
            {
               stories.filter(story=>(localStorage.getItem('following').includes(story.userId)||story.userId===localStorage.getItem('userId'))&&Math.abs((new Date().getTime()-new Date(story.createdAt).getTime())/(1000*60*60))<24).map(story=>(
                <div className="story user-story" key={story._id} onClick={()=>handleOpenStory(story)}>
